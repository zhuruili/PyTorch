...
# torch.save(cnn.state_dict(), 'Rookie\part_2_items\MachineLearning\PyTorch\model\cnn2.pkl')#保存模型

# 加载模型，调用时需将前面训练及保存模型的代码注释掉，否则会再训练一遍
cnn.load_state_dict(torch.load('Rookie\part_2_items\MachineLearning\PyTorch\model\cnn2.pkl'))
cnn.eval()
