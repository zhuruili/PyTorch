...
# 超参数设置
data_path =  'D:\helloworld_python\LSTM\data\data.txt'              # 数据集
vocab_path = 'D:\helloworld_python\LSTM\data\\vocab.pkl'             # 词表
save_path = 'D:\helloworld_python\LSTM\saved_dict/lstm.ckpt'        # 模型训练结果
embedding_pretrained = \
    torch.tensor(
    np.load(
        'D:\helloworld_python\LSTM\data\embedding_Tencent.npz')
...
def plot_acc(train_acc):
    ...
    plt.ylabel('Acc')
    plt.legend(loc='best')
    plt.savefig('D:\helloworld_python\LSTM\\results\\acc.png', dpi=400)
...
def plot_loss(train_loss):
    ...
    plt.ylabel('loss')
    plt.legend(loc='best')
    plt.savefig('D:\helloworld_python\LSTM\\results\loss.png', dpi=400)
...
def result_test(real, pred):
    ...
    plt.savefig("D:\helloworld_python\LSTM\\results\\reConfusionMatrix.tif", dpi=400)
